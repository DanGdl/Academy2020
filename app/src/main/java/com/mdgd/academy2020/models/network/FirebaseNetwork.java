...
public class FirebaseNetwork implements Network {
    ...
    @Override
    public Single<Result<String>> createNewUser(String email, String password) {
    ...
    public Single<Result<Boolean>> updateUser(UserData data) {
        ...
        return execRequest(firebase.collection(USERS).document(firebaseAuth.getCurrentUser().getUid()).set(params, SetOptions.merge()))
                .map(updateResult -> {
    ...
    public Single<Result<UserData>> getUser() {
        return execRequest(firebase.collection(USERS).document(firebaseAuth.getCurrentUser().getUid()).get())
                .map(userResult -> {
                    if (userResult.isFail()) {
